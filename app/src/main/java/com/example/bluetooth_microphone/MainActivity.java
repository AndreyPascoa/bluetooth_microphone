package com.example.bluetooth_microphone;
...
public class MainActivity extends AppCompatActivity {
        ...
        if (audioManager != null) {
            ...
            if (!audioManager.isBluetoothScoOn()) {
                try {
                    ...
                    // Mude o modo de áudio para comunicação antes de iniciar o SCO
    ...
    private void startRecording() {
        ...
        try {
            // Cria um Uri no MediaStore
            Uri uri = getContentResolver().insert(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, values);

            // Abra um ParcelFileDescriptor para o URI
            ParcelFileDescriptor pfd = getContentResolver().openFileDescriptor(uri, "w");

            mediaRecorder = new MediaRecorder();
            mediaRecorder.setAudioSource(MediaRecorder.AudioSource.VOICE_COMMUNICATION); // SCO audio
            mediaRecorder.setOutputFormat(MediaRecorder.OutputFormat.THREE_GPP);
            mediaRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AMR_NB);

            // Use o descritor de arquivo do ParcelFileDescriptor
